...
}) =>
{
  ...
  const fs = require("fs");
  var juswa = args.join(" ");
  if (!juswa) return api.sendMessage(`Please Enter Your Name`, event.threadID, event.messageID);
  else
  {
    axios.get(`https://cakrayp.herokuapp.com/api/textmaker/textprome/create?text=${encodeURIComponent(juswa)}&url=https://textpro.me/create-a-3d-orange-juice-text-effect-online-1084.html&apikey=cakrayp24Q6&responsetype=json`).then(res =>
    {
